...
void LoG_filter(RGBTRIPLE ***output_image_data, RGBTRIPLE ***image_data, IMAGEINFO *image_info, double sigma, int kernel_size)
{
    ...
    for(k = -(kernel_size-1)/2; k <= (kernel_size-1)/2; k++) {
        for(l = -(kernel_size-1)/2; l <= (kernel_size-1)/2; l++) {
            printf("%2.4f ", filter[k+(kernel_size-1)/2][l+(kernel_size-1)/2]);
        }
        printf("\n");
    }
    filter_sum -= filter[(kernel_size-1)/2][(kernel_size-1)/2];
    filter[(kernel_size-1)/2][(kernel_size-1)/2] = -1 * (kernel_size-1) * (kernel_size-1);
    filter_sum = filter[(kernel_size-1)/2][(kernel_size-1)/2] / filter_sum;
    for(k = -(kernel_size-1)/2; k <= (kernel_size-1)/2; k++) {
        for(l = -(kernel_size-1)/2; l <= (kernel_size-1)/2; l++) {
            if(k != 0 || l != 0) {
    ...
    for(k = -(kernel_size-1)/2; k <= (kernel_size-1)/2; k++) {
        for(l = -(kernel_size-1)/2; l <= (kernel_size-1)/2; l++) {
            printf("%2.2f ", filter[k+(kernel_size-1)/2][l+(kernel_size-1)/2]);
        }
        printf("\n");
    }
